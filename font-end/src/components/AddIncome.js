...
onSubmitForm=(e)=>{e.preventDefault();
    ...
    axios.post('http://localhost:4000/ems/add',newIncome)
...
catGroup=()=>{ 
 ...
 return this.state.emscat.map((currentEmCat,index)=>{  if(currentEmCat.type===this.state.type){
      return <Row key={index}>
          <Col><input type="radio" onChange={this.onChangeCategory} value={currentEmCat.newcategory} 
                       checked={this.state.category===currentEmCat.newcategory} />{currentEmCat.newcategory} </Col>
...
changeh=()=>{
    ...
    render() { if(this.state.dir===true)return<Redirect to="/"/>
        return ( 
         ...
         <div className="border border-success" style={{background:"lightgreen"}} >
            <form onSubmit={this.onSubmitForm} style={{margin:15}}>
               <Container>
                  <div> <h5>Select Category for Income </h5>
                   {this.catGroup()}
                  <Row>
                 <Col><input type="text" placeholder="Ex: Tea" onChange={this.onChangeNewCategory}/>
                 din't find the Category Add here new Category</Col>
                 </Row><br></br>
                 </div> 
